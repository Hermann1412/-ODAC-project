...
class LoginWindow(QMainWindow):
	...
	def go_to_register_page(self):
		webbrowser.open('https://server-side-deployment-8m3n.onrender.com/register/')

	# Login function that manages the token authentication
	def login(self):
		try:
			url = 'https://server-side-deployment-8m3n.onrender.com/api/get_auth_token/'
			...
			# Bad response
				...
				# Show error
				self.popup.setText("Fill the blank or else your Username or Password is not correct")
				self.popup.exec_()
		except:
